...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_normalized_running = (x - running_mean) / np.sqrt(running_var + eps)
    
    out = gamma*x_normalized_running + beta 
    
    cache = (x, running_mean, running_var, eps, x_normalized_running, gamma, beta)
...
def batchnorm_backward(dout, cache):
  ...
  x, mean, var, eps, x_normalized, gamma, beta = cache 
  ...
  p1 = 1 / np.sqrt(var + eps) 
  dl_dxhat = gamma*dout 
  dx1 = p1*dl_dxhat 
  
  m = x.shape[0]
  p2 = (2/m)*np.ones(x.shape)*(x - mean)
  dl_dvar = -0.5 * (var + eps)**-1.5 * np.sum( (x - mean)*dl_dxhat, axis=0 )
  dx2 = p2*dl_dvar 
  
  dl_dmu = (-1 / np.sqrt(var + eps))*np.sum(dl_dxhat, axis=0)
  dx3 = (1/m)*dl_dmu 
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    out = x # don't calculate mask at test time 
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
