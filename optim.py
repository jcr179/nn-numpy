...
def sgd_nesterov_momentum(w, dw, config=None):
  ...
  v0 = v 
  v = 0.9*v - config['learning_rate']*dw
  next_w = w + v + 0.9*(v - v0) # first 2 terms: original gradient, last term: accounting for 'step forward'
...
def rmsprop(w, dw, config=None):
  ...
  config['a'] = beta*config['a'] + (1 - beta)*np.multiply(dw, dw)
...
def adam(w, dw, config=None):
  ...
  v_tilde = config['v'] / (1 - config['beta1']**config['t'])
  a_tilde = config['a'] / (1 - config['beta2']**config['t'])
  
  next_w = w - config['learning_rate'] * v_tilde / (np.sqrt(a_tilde) + config['epsilon'])
